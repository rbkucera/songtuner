...
function segmentColor(dist: number) {
  if (dist <= 2) return { active: '#00e644', dim: 'rgba(0,230,68,0.07)' };
  if (dist <= 5) return { active: '#ff7700', dim: 'rgba(255,119,0,0.07)' };
  return { active: '#ff2200', dim: 'rgba(255,34,0,0.07)' };
}

export function PitchBlckMeter({ centsOff, inTune }: PitchMeterProps) {
  ...
  return (
    <div className="w-full px-4">
      <div className="flex justify-between mb-2" style={{
        fontFamily: "'Orbitron', system-ui, sans-serif", fontSize: '10px', letterSpacing: '0.15em', color: '#6a6258',
      }}>
        <span>FLAT</span>
        <span>SHARP</span>
      </div>
